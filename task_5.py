...
def e5learnednotrandombaseline(train_data: Dataset, test_data: Dataset, config: Config):
    ...
    for epoch in tqdm(range(config.epochs), desc="Training model"):
        ...
        for i in range(0, len(train_questions), config.batch_size):
            ...
            neg_sim = torch.mm(torch.Tensor(question_embeddings),
                               torch.Tensor(answer_embeddings).T)
            neg_indices = []
            for j in range(i, end):
                neg_sim[j][j] = -numpy.inf
                neg_indices.append(numpy.argmax(neg_sim[j]))
            ...
            loss = loss_fn(question_embeddings, answer_embeddings, negative_embeddings)
